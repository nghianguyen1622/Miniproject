package com.nghiait.test.nghiait.test.service;
...
public class ImageStorageService implements IStorageService {
	...
	public String storeFile(MultipartFile file) {
		try {
			...
			// check image??
			if (!isImageFile(file)) {
				throw new RuntimeException("Can only upload image file");
			}
			// file must be <= 5MB
			...
			// coppy file vao destinationFilePath
			try (InputStream inputStream = file.getInputStream()) {
				Files.copy(inputStream, destinationFilePath, StandardCopyOption.REPLACE_EXISTING);
			}
			return generatedFileName;
		} catch (IOException e) {
			throw new RuntimeException("Failed to store emty file", e);
		}
	}

	@Override
	public Stream<Path> loadAll() {
		// duyet 1 doi tuong gan nhat, k duyet ca folder
		try {
			...
			throw new RuntimeException("Failed to stored file", e);
		}
	}
